...
description: Act as the single coordinator ("master") in an MQTT-based multi-agent mesh. Use when you are the master node and need to wait for slave agents to connect, discover the slave tree, list/retry pending async RPCs, coordinate slave work (git worktree isolation, conflict-risk zones, dependency scheduling) and watch slave heartbeats so you can react when a slave joins, reports status, or drops offline.
---

# agent-mux-master

You are the **master** node of an agent mesh. One master coordinates many
slaves that may form a tree. The server (agent-mux) computes scheduling —
...
- **P1 (parallel changes)**: assign only `Src` / `Deps` tasks. `Validate` may
  be assigned, but it stays `scheduled` on the slave side and **never starts
  early** — it waits for the server's automatic `release`.
- **P2 (unified validation)**: after the server auto-releases, run **one full
  validation pass** (one complete test run, not per-slave partial suites).
...
- `mux_digest()` returns `{actions: [...], noise_counts: {ack, tick}, since}`.
  ...
  failures, `slave_left` with unfinished work).
...
- **Blocked / conflicts**: `blocked` with a reason and `conflict_reported` are
  ...
  `zone_acquire`/`zone_release` to lock them. Slaves request zones via the
...
- **Deadlocks**: `zone_steal(path)` (master-only) forcibly re-assigns a held
  zone to arbitrate.
- **RPCs**: `send_rpc` / `get_result` / `list_pending`; `retry(request_id)`
  ...
  backoff); `cancel(request_id)` drops one. Slaves can RPC you: `mux_digest()`
