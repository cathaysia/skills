> **2026-08-13 修正（当前方向）**：manager 的职责是**协调 executor 自己带来的工作**（通常由用户 spawn executor 时指派），避免并行冲突 —— 不是给 executor 派活，也不是"server 从 assign 派任务"。`assign` 只用于协调需要的**改派/引导**（原 owner 掉线、去冲突），不是工作开始的正常方式。本稿保留 A1/A3/A4/A5 与事件分类、审批门、zone 权威化、可靠性设计；A2 的任务表改为"改派/引导时由 server 记账"，executor 的常规工作不经过 assign。
